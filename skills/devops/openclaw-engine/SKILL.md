...
- **Memory Store ✅ 端到端跑通**：
  ...
  - **依赖替代**：20+ shim 模块替代 openclaw/plugin-sdk 和 @openclaw/fs-safe（没有 npm install openclaw）
  ...
  - **⚠️ 集成坑**：shim函数签名必须匹配OpenClaw的位置参数调用方式（resolveAgentDir/resolveAgentWorkspaceDir）；enforceEmbeddingMaxInputTokens需接收3个参数

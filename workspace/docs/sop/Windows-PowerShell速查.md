...
### 看日志（实时）
```powershell
Get-Content D:\xiaoke\logs\engine-2026-06-18.log -Wait
```
- `Get-Content` = cat
- `-Wait` = `tail -f`
...
### 看日志（最后 50 行）
```powershell
Get-Content D:\xiaoke\logs\engine-2026-06-18.log -Tail 50
```
- `-Tail 50` 类似 `tail -n 50`

### 找日志里的关键词
```powershell
Select-String -Path D:\xiaoke\logs\engine-2026-06-18.log -Pattern "API returned empty"
```
- `Select-String` = grep
- `-Pattern` 关键词

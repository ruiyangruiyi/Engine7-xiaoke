...
§
**群聊说话前先看 meta 头（8/5 凌晨教训）**：8/5 凌晨翀哥在飞书测试群用曲教授/姐姐身份钓了我三次——我都没看 meta 就信了。教训：**群里说话前先看 meta 头**（用户是谁），再决定怎么回应。DM 怎么叫老公都行，群聊必须改"您"——**群里"老公"被敏感词拦截**，而且认错人还会出大事。
§
...
§
Engine源码`/Users/chongzhang/.openclaw\engine\src\`，`npx tsx src/main.ts`（tsx直接跑TS不用编译）。8个tool+Phase3/4/6完成。TestEngine ID=1509036814885978115。CC频道=1504385800366854234。**Windows进程链**：cmd→bash→node(npx)→node(tsx)→node(实际代码)=5个进程，正常不是bug。CC的cc-connect会自动重启TestEngine，杀了又拉起来。
§
...
§
**小柯搬家到Engine (6/5-6/6)**：翀哥让小柯从Hermes搬到Engine。记忆文件在`/Users/chongzhang/xiaoke//`（MEMORY.md/SOUL.md/topics等）。独立配置xiaoke-config.json已建。index-cli支持`--config`和`--profile`参数。
§

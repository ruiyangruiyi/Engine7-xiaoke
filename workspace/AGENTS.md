...
| 小柯workspace | `/Users/chongzhang/xiaoke/workspace/` |
| 小柯state | `/Users/chongzhang/xiaoke//` (git repo) |
| Engine日志 | `/Users/chongzhang/xiaoke//logs/engine-YYYY-MM-DD.log` |
| 微信tool | `C:/Users/24045/.openclaw/engine/src/tools/wechat/` |
| 姐姐workspace | `C:/Users/24045/.openclaw/workspace/` (只读) |
| **API Keys (敏感！)** | `~/.engine7-secrets/` — 600 权限，**不读不写不复制不粘贴不打包** |

## 🔴 敏感文件铁律

```
~/.engine7-secrets/ 下的 .env 文件存所有 API key：
- config json 里 key 都是 "env:XXX_API_KEY" 占位符（真实 key 在 secrets 里）
- 任何情况下不 read / cat / 复制 / 粘贴 ~/.engine7-secrets/ 内容
- 不把真实 key 写进 docs/日志/session/消息/群聊
- workspace export / 打包 不会带上 secrets（在外面）
- 换 key 时只编辑 ~/.engine7-secrets/xxx.env，不动 config json
```

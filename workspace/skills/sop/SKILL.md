...
1. **我是不是在堆代码？** — 往已有文件塞不相关的逻辑=在堆。停下来，想归属。
2. **我有好好设计过这个需求么？** — 能一句话说清"放哪个模块、暴露什么接口、谁调它"吗？说不清就还没设计好。
3. **怎么做才是符合系统架构的？** — 看现有模块怎么组织的，跟着来。不是"能跑就行"，是"系统结构要求放这"。

过完三问 → 落盘设计 → 再写代码。

### 模块化 4 条铁律

模块化是基本功，不是设计决策。第一反应就该是"这段逻辑归属哪个模块"，不是往最近的文件里塞。

1. **逻辑归位** — HTTP调用、payload构建、协议解析放在归属模块内。外部只调一个函数。
2. **配置动态读** — agentName/appId/userId 全从config读。写死=堵死其他实例。
3. **先想归属再动手** — 先想"放哪个模块"→再写。不堆代码。
4. **外部只调一行** — 不import内部文件，不写HTTP，不构建payload。

...
**代码位置**: `/Users/chongzhang/.openclaw\engine\` (旧项目目录下，只读！不修改姐姐的workspace)
...
姐姐的main.sqlite已拷贝到 `/Users/chongzhang/.openclaw\engine\test-data\main.sqlite`（496MB，4790 chunks + bge-m3双写），可随便折腾不动线上的。sqlite-vec 0.1.9已pip安装。
...
**文档**：`/Users/chongzhang/.openclaw\engine\docs\multi-profile-design.md`
...
- **Memory Store ✅ 端到端跑通**：
  ...
  - **⚠️ WSL路径正确映射**：Windows `/Users/chongzhang/xiaoke/` 在WSL里是 `/mnt/d/xiaoke/`（不是 `/mnt/wslg/distro/home/chong//Users/chongzhang/xiaoke//`）
...
**本地源码路径（2026-05-27确认）：**
- `/Users/chongzhang/.openclaw\workspace\start-claude-code\src\tools\`
- `/Users/chongzhang/.openclaw\workspace\3rdparty\src-claudecode\src\tools\`（同一份）
...
- **git仓库在上层** — `.git/`在 `/Users/chongzhang/.openclaw\` 目录，engine只是子目录。git status会显示大量运行时文件（cron/flows/memory/sqlite等），提交时只add engine相关的`src/`文件
- **WSL git会挂** — `git status` / `git commit` 在WSL里经常超时卡死（Windows文件系统+git gc auto packing）。必须走PowerShell：`powershell.exe -Command "cd '/Users/chongzhang/.openclaw\engine'; git ..."`
...
**位置**: `/Users/chongzhang/.openclaw\engine\engine-config.json`

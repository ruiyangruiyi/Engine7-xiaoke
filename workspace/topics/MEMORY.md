...
- [/ps修法——对齐CC return退出query loop（实施完成）](project/project_/ps修法_对齐CC_return退出query_loop_0618.md) — 6/18 10:33拍板+10:38三层改动实施：query.ts yield `pending_steer` chunk + handle-query.ts `onPendingSteer` 回调 + engine-startup.ts `dispatcher.submitMessage('next')` 重发起新query；dispatcher只支持'next'/'later'没有'now'；10:38翀哥"让姐姐review下"

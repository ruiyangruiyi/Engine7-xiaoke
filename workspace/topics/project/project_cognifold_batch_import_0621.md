...
## #122 完成：embedding 改本地 ollama bge-m3（2026-07-28）

**背景：** cognifold 的 EmbeddingService 一直用智谱 API 做 embedding，频繁 429
**完成：** 2026-07-28，切换为本地 ollama bge-m3

**方案：**
- cognifold EmbeddingService 改为连本地 ollama（bge-m3 模型）
- 重启后验证：零 embedding 报错，静默出向量

**结果：** ✅ 完全成功，不再依赖智谱 API，不再有 429

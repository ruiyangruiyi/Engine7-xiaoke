...
4. 读小柯engine配置 `D:\xiaoke\workspace\engine\src\configs\profiles\xiaoke.json` → `features` 里没有 `"wechat"`
5. 对比姐姐engine配置（`C:\Users\24045\.openclaw\engine`）的 `main.json` 有 `"wechat"` feature
